...
from rootski_api.tests.fixtures.seed_data import seed_data, EXAMPLE_BREAKDOWN_771
...
def test__get_breakdown(dynamo_client: TestClient, dynamo_db_service: DynamoDBService):
    ...
    word_id = int(EXAMPLE_BREAKDOWN_771["word_id"])
    ...
    assert response["word_id"] == int(EXAMPLE_BREAKDOWN_771["word_id"])
    assert response["word"] == EXAMPLE_BREAKDOWN_771["word"]
    assert response["is_verified"] == EXAMPLE_BREAKDOWN_771["is_verified"]
    assert response["is_inference"] == EXAMPLE_BREAKDOWN_771["is_inference"]
